iprior_em_closed <- function(mod, maxit = 500, stop.crit = 1e-5, silent = FALSE,
                             lambda0 = NULL, w0 = NULL, psi0 = NULL) {
  ...
  niter <- 0
  loglik <- rep(NA, maxit)
  loglik[1] <- -n / 2 * log(2 * pi) - logdet / 2 - crossprod(y, Vy.inv.y) / 2
  ...
  while (em_loop_logical()) {
    # UPDATE LAMBDA
    for (k in seq_len(p)) {
      lambda <- expand_Hl_and_lambda(lambda, lambda, intr, intr.3plus)$lambda
      BlockB(k)
      T1 <- sum(Psql[[k]] * W)
      T2 <- crossprod(y, Pl[[k]]) %*% w - sum(Sl[[k]] * W) / 2
      lambda[k] <- as.vector(T2 / T1)
    }
...
em_loop_logical <- function() {
  ...
  ll.diff <- loglik[niter] - loglik[niter - 1]  # will also stop when loglik becomes smaller
  crit1 <- (niter != maxit)
  crit2 <- (ll.diff > stop.crit)
